...
const char* ssid  =  "xxxx";     // SSID WiFi network
const char* pass  =  "xxxxxxxxx";     // Password  WiFi network
const char* token =  "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx";  // Telegram token
int64_t userid = xxxxxxxxx;
  ...
  if (myBot.getNewMessage(msg)) {
    ...
    if (msgType == MessageText){
      ...
      if (msgText.equalsIgnoreCase("/picfs1")) 
      {
        ...
        myBot.sendPhoto(msg, "/telegram-bot2.jpg", FILESYSTEM);
      }
      ...
      {
        Serial.println("\nSending text file from filesystem");
        File file = FILESYSTEM.open("/new_file.txt", "r");
        //myBot.sendPhoto(msg, file, file.size());
        myBot.sendText(msg, file, file.size());
        //myBot.sendStream(msg.chatId, "sendPhoto", "image/jpeg", "photo", file, file.size());
       // myBot.sendDocument(&msg, file, file.size(), &doc);
        file.close();
      }
      ...
      // Send a picture passing url to online file
        ...
        replyMsg += "/picfs1 or /picfs2 will send an example picture or text file from fylesystem\n";
